...
console.log(`Party: ${partyId.substring(0, 40)}...`);
...
const request = {
  commands: {
    ...
    act_as: [partyId],
    // Let Canton auto-select synchronizer - don't specify it!
    commands: [{
      create: {
        template_id: {
...
client.submitAndWait(request, metadata, (error, response) => {
  if (error) {
    ...
    console.error('Full error:', JSON.stringify(error.metadata?.internalRepr, null, 2));
    process.exit(1);
  }
  
  console.log('✅ Employee contract created!');
  
  if (!response.transaction?.events?.[0]?.created?.contract_id) {
  ...
  // Wait a moment for the contract to settle
  setTimeout(() => {
    const exerciseReq = {
      commands: {
        application_id: 'payroll-test',
    ...
    console.log('Submitting exercise command...');
    client.submitAndWait(exerciseReq, metadata, (error, response) => {
      ...
      console.log('\n🎉 🎉 🎉 SUCCESS! 🎉 🎉 🎉');
      console.log('✅ PaySalary choice exercised!');
      console.log('✅ PaymentRequest contract created on Canton!');
      console.log('\n👀 CHECK YOUR BRIDGE TERMINAL FOR PAYMENT EXECUTION! 👀');
      
      // Show some transaction details
      if (response.transaction?.events) {
        console.log(`\n📊 Transaction created ${response.transaction.events.length} events`);
      }
