...
// Correct Canton command structure
const request = {
  commands: {
...
client.submitAndWait(request, metadata, (error, response) => {
  ...
  if (!response.transaction || !response.transaction.events || response.transaction.events.length === 0) {
  ...
  client.submitAndWait(exerciseReq, metadata, (error, response) => {
    ...
    console.log('👀 Check your bridge terminal for payment execution!');
    process.exit(0);
  });
});
